...
- The software-stacked frame (`R11–R4`) is filled with recognizable placeholder values, useful for spotting stack corruption in a debugger.
...
Context switches happen in `Core/Src/pendsv_handler.s`, triggered by setting `PENDSVSET` in the `SCB->ICSR` register (see `os_yield()`, `os_sleep_ms()`, `os_task_block()`). `PendSV` is deliberately given the lowest interrupt priority (`os_init()` sets it to `0xFF`) so it always runs after any other pending interrupt has completed — this is the standard ARM pattern for avoiding a context switch mid-ISR.
